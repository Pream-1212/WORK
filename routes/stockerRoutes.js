...
router.get("/stockreport", async (req, res) => {
  try {
    ...
    res.render("stockreport", { stockreport }); //  You must pass { stocks }
  } catch (error) {
    console.error(error.message);
    res.render("stockreport", { stockreport: [] }); // fallback so pug doesn't break
  }
});
...
router.post("/stockreport", async (req, res) => {
  ...
  try {
    const stockreport = new Stocker(req.body);
